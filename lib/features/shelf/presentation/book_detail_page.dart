...
class _BookDetailPageState extends ConsumerState<BookDetailPage> {
  ...
  Future<void> _loadToc({bool forceRefresh = false}) async {
    ...
    // 2. 在线拉取完整真实目录
    try {
      // findByName 返回可空，但 ?? 后面的 firstWhere 带 orElse 一定给得出结果，
      // 所以这里本就非空；声明成可空会让后面的重新赋值破坏类型提升。
      SourceRule rule = BuiltinSources.findByName(_book.sourceName) ??
          BuiltinSources.all.firstWhere(
            (s) => s.id == _book.sourceId,
            orElse: () => BuiltinSources.all.first,
      ...
      // 此前这里还有一段写死的「书名 → URL」兜底映射。那些链接和榜单里的
      // 一样会过期：书源站的数字 ID 随新书插入而重排，实测 16 本里 12 本
      // 已经指向了别的书。一律改成按书名现搜，拿当前真实的链接。
      // `biquge.company` 是早期占位域名，同样视为无效。
      if (targetUrl == null ||
          targetUrl.isEmpty ||
        ...
        try {
          ...
          final matched = pickBestBookMatch(searchResults, cleanTitle,
              author: _book.author);
          if (matched != null) targetUrl = matched.bookUrl;
        } catch (_) {}

        // 首选源没有这本书就全网并发找，否则换源前用户只能看到一页空目录
        if (targetUrl == null || targetUrl.isEmpty) {
          try {
            final allResults = await _multiSourceService.searchAll(cleanTitle,
                timeout: const Duration(seconds: 6));
            final matched =
                pickBestBookMatch(allResults, cleanTitle, author: _book.author);
            if (matched != null) {
              targetUrl = matched.bookUrl;
              final matchedRule = BuiltinSources.findById(matched.sourceId);
              if (matchedRule != null) rule = matchedRule;
            }
          } catch (_) {}
        }
      }
              ...
              Expanded(
                child: ListView.separated(
                  ...
                  separatorBuilder: (_, __) => Divider(
                      height: 0.5, thickness: 0.5, color: colors.borderSubtle),
          ...
          slivers: [
              ...
              flexibleSpace: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final expandRatio = collapseRange > 0
                      ...
                      : 0.0;
                  final titleOpacity =
                      (1.0 - expandRatio * 2.2).clamp(0.0, 1.0);
                ...
                child: SoftCard(
                  ...
                  child: Row(
                    children: [
                      ...
                      _buildStatItem(colors, '篇幅', readWordCount,
                          isGold: false),
                    ...
                    Expanded(
                      ...
                      child: GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color:
                                _isInShelf ? colors.accentSoft : colors.surface,
            ...
            SliverPersistentHeader(
              ...
              delegate: _BookDetailTocHeaderDelegate(
                colors: colors,
                totalCount: _chapters.isNotEmpty
                    ? _chapters.length
                    : totalChaptersCount,

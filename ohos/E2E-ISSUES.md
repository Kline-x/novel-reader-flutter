...
门禁：`flutter analyze` 0 issues、`flutter test` 238/238。

### 真机回归（同一台 HBN-AL00）

| 验证项 | 结果 |
|---|---|
| 版本号 | 日志 `getPackageInfo -> 1.0.9+4007`，设置页显示「当前 v1.0.9」 |
| 鸿蒙身份 | 日志 `已读取真实安装版本: 1.0.9+4007 (HarmonyOS)` |
| 文件选择器 | 鸿蒙原生「安全访问文件」界面正常拉起，带格式过滤 |
| **分章修复** | 同一份 3 章样本重新导入：**进度显示 33%（3 章）**，修复前是 20%（5 章）；「第二章 传书验证」标题正确、正文完整留在章节内 |
| 音量键开关 | 已置灰，副标题显示「鸿蒙系统独占音量键，应用无法接管」 |
| 排版面板配色 | 强调色已跟随应用主题 |
| 配色持久化 | 切「翠竹微雨」→ 覆盖安装 → 仍是翠竹微雨（问题 10 不复现） |

注意：分章修复只作用于**新导入**的书。已入库书籍的目录是导入时算好存下的，
不会自动重切——要修正得重新导入。
